...
const Msg = require('./inGameUtils/Msg.js')
const Item = require('prismarine-item')('1.12.2')
// const CreateParser = require("./parser.js").load;
const Vec3 = require('vec3')
const PlayerEntity = require('./inGameUtils/playerentity.js')
const { MediaPlayer } = require('./mapUtils/media')
...
function spawnDisplay (client, x, y, z, mapID = 1, frameEID) {
  ...
  const item = Item.toNotch(new Item(358, 1))
...
module.exports = function () {
  ...
  const handler = CreateHandler(serv)
  ...
  serv.on('login', (client) => {
    ...
    client.on('chat', ({ message }) => {
      if (message.startsWith('/')) {
        ...
        serv.chat([new Msg(client.username, 'gold'), new Msg(': ', 'reset'), new Msg(message, 'reset')])
