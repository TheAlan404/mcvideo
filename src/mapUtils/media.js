...
class MediaPlayer extends EventEmitter {
  ...
  createVideoStream (src) {
    startTime = Date.now()
    const sourceStream = this.getSource(src)
    ...
    const splittedStream = new StreamSplitter(PNGHEADER)
  ...
  async renderImage (src) {
    ...
    if (!Buffer.isBuffer(src)) {
      if (fs.existsSync(src)) {
        ...
        try {
          const req = await fetch(src) // TODO: fix TypeError: Only absolute URLs are supported (Luna)
    ...
    const displays = await this.divideToScreens(src)
    for (const n in displays) {
  ...
  async divideToScreens (pngFile) {
    ...
    for (let y = 0; y < this.displays.height; y++) {
      for (let x = 0; x < this.displays.width; x++) {
        i++
        const data_RGBA = ctx.getImageData(x * 128, y * 128, 128, 128).data
        const data = new Uint8ClampedArray((128 * 128) * 3)
        let j = 0
