...
module.exports = function (serv, Parser) {
  ...
  handler.on('incorrectUsage', function (cmd, usage, client) {
  ...
  handler.addCommand(new Command({
    ...
    usage: [],
    run: (args, client) => {
      const src = 'https://www.youtube.com/watch?v=UnIhRpIT7nc'
  ...
  handler.addCommand(new Command({
    ...
    usage: [':color number'],
    run: (args, client) => {
      const h = Buffer.alloc(128 * 128).fill(Buffer.from(args[0], 'hex'))
